...
const gatosFiltrados = computed(() => {
  return gatos.value.filter((gato) => {
    ...
    const coincideCastrado = !filtroCastrado.value || gato.castrado === filtroCastrado.value;
...
<template>
  <div class="container mt-5 gatos-page">
    ...
    <div v-if="gatosPaginados.length" class="row">
      <div v-for="gato in gatosPaginados" :key="gato.id" class="col-12 col-sm-6 col-lg-3 mb-4">
        <div class="card h-100 shadow-sm gato-card">
          <img :src="buildCatImage(gato.imagen)" class="card-img-top gato-imagen" :alt="`Imagen de ${gato.nombre}`" />
          ...
          <div class="card-body d-flex flex-column">
            ...
            <div class="mt-auto d-flex gap-2">
              <button class="btn btn-adoptar flex-fill" @click="adoptar(gato.id)">Adoptar</button>

              <button class="btn btn-acoger flex-fill" @click="adoptar(gato.id)">Acoger</button>

              <RouterLink :to="`/detalleGato/${gato.id}`" class="btn btn-detalle flex-fill"> Ver más </RouterLink>
            </div>
          </div>
        </div>
      </div>
    ...
    <p v-else class="text-center mt-4">Ahora mismo no hay gatetes disponibles.</p>

    <nav v-if="totalPaginas > 1" class="d-flex justify-content-center mt-4" aria-label="Paginación de gatos">
      <ul class="pagination">
  ...
  <div v-if="mostrarModalAuth" class="modal-overlay">
    <div class="auth-modal">
      ...
      <p>Inicia sesión o regístrate para enviar solicitudes de adopción.</p>

      <div class="modal-actions">
        <RouterLink :to="`/iniciarSesion?redirect=/adoptar/${gatoPendiente}`" class="modal-btn primary"> Iniciar sesión </RouterLink>
